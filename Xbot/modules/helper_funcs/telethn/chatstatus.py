from Xbot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Xbot import DRAGONS
...
async def IronMen_is_admin(chat_id: int):
    status = False
    IronMen = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if IronMen.id == user.id:
            status = True
            break
    return status
